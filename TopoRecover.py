...
def handle_devices_section(settings: dict, settings_path: Path) -> bool:
    """
    Interaktive Verwaltung des ``devices``-Abschnitts in den Einstellungen.

    Die Funktion listet zunächst alle aktuell konfigurierten Geräte auf und
    ermöglicht dem Benutzer anschließend über eine Auswahlmenü-Aktion
    Geräte hinzuzufügen, zu bearbeiten oder zu löschen. Anschließend wird
    der aktualisierte ``devices``-Block wieder in die übergebene
    ``settings``-Struktur geschrieben und als JSON-Datei gespeichert.

    Aktionen:
        - **add**:  Fügt ein neues Gerät mit IP, Port, Gerätetyp,
            device_ios, Benutzername und Passwort hinzu. Falls für eine IP
            der gewählte Port bereits existiert, wird der Vorgang abgebrochen.
        - **edit**: Bearbeitet die Eigenschaften (``device_type``,
            ``device_ios``, ``username``, ``password``) eines ausgewählten
            Geräts.
        - **remove**: Entfernt ein ausgewähltes Gerät. Wenn für eine IP
            nach dem Entfernen kein Port mehr vorhanden ist, wird der gesamte
            Eintrag für diese IP gelöscht.

    Side Effects:
        - Interaktive Benutzereingaben über die Konsole mittels ``click``.
        - Ausgabe von Status- und Hilfetexten über ``click.echo``.
        - Überschreiben der Datei unter ``settings_path`` mit den
        aktualisierten JSON-Einstellungen.
